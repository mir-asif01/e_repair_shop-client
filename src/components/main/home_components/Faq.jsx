...
function Faq() {
    ...
    return (
        <div className=" rounded-lg font-sans md:m-10 p-3 md:p-10">
            {accordionsData.map((PerAccordion, idx) => (
                <div key={idx} className="border-b border-gray-500 last-of-type:border-none">
                    <button onClick={() => toggle(idx)} className="flex h-full w-full items-center justify-between py-4 font-medium dark:text-black">
                        <span className="sm:text-lg md:text-xl">{PerAccordion.title}</span>
                        <span className="rounded-full p-2 ">
                            <svg className="ml-8 mr-7 shrink-0" width="16" height="16" xmlns="http://www.w3.org/2000/svg">
